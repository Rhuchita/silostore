...
class ItemForm(forms.ModelForm):
    class Meta:
        category_choices = [
            ...
            ('deskstationery', 'Desk Stationary'),
            ('markers', 'Markers'),
            ('pencils', 'Pencils'),
            ('pens', 'Pens'),
            ('uniforms', 'Uniforms')
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        '''description = str(request.POST.get("description", False))
        print(Item.objects.filter(title=listing_title,
              description=description).get()) '''
        item_ = Item.objects.filter(
            title=listing_title).get()
    ...
    if items != None:
        items_with_bids = Item.objects.filter(
            user_bid_items__items__isnull=False).distinct()
        bids = []
        for item in items:
            if item in items_with_bids:
                bids.append(Bid.objects.filter(
                    items=item).order_by('amount').last().amount)
            else:
                bids.append(item.starting_bid)
        logger.info(bids)
        items_bids = list(zip(items, bids))
        return render(request, "silostore_web/index.html", {
            "items": items,
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            return HttpResponseRedirect(reverse("silostore_web:index"))
        else:
            return render(request, "silostore_web/login.html", {
...
def register(request):
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(request, "silostore_web/register.html", {
...
def add(request):
    if request.method == 'GET':
        logger.info("Get add")
        return render(request, "silostore_web/add.html", {
            "new_form": ItemForm()
        })
    else:
...
def cards_view(request):
    ...
    return render(request, "silostore_web/cards.html", {
        "user": user,
        "item": item,
...
def listing_details_view(request):
    ...
    if request.method == 'POST':
        ...
        if len(search_bid) == 0 and message_bid != "Couldn't place the bid.":
            return render(request, "silostore_web/listing_details.html", {
                "item": item_,
                "owner": owner,
                "comments": comments,
...
def add_comment(request):
    ...
    if request.method == 'POST':
        ...
        new_comment.save()
    return HttpResponseRedirect(reverse("silostore_web:listing_details")+"?listing_title="+item_name+"")
...
def watchlist_view(request):
    ...
    return render(request, "silostore_web/watchlist.html", {
        "watchlist": watchlist
    })
    # items = utility.get_items()
    # distinct_users = utility.get_users_with_items()
    ...
    for item in _items:
        if item.category == _category and item.status == True:
            if item in intersection_list:
                bids_for_items.append(Bid.objects.filter(
                    items=item).order_by('amount').last().amount)
            else:
                bids_for_items.append(item.starting_bid)
    items_bids = list(zip(_items, bids_for_items))

    return render(request, "silostore_web/category_listing.html", {
        "category": _category,
        "items": _items,
        "items_bids": items_bids
    })
...
def category_list(request):
    category_choices = [
        ...
        ('pens', 'Pens'),
        ('uniforms', 'Uniforms')]
    return render(request, "silostore_web/categories.html", {
...
def add_to_watchlist(request):
    if request.method == 'POST':
        ...
        if len(search_item) != 0:
            ...
            message = "Item Has already been added to watchlist"
        return HttpResponseRedirect(reverse("silostore_web:index", kwargs={"message": 'Hello'}))
...
def place_bid(request):
    ...
    return HttpResponseRedirect(reverse("silostore_web:listing_details")+"?listing_title="+listing_title+"&message_bid="+message+"")
    ...
    if len(auction_history) != 0:
        ...
        for auction_item in auction_history:
            bid_item = Bid.objects.filter(
                items=auction_item.items, user=user_).order_by('amount').last()
            bids.append(bid_item)
            items.append(auction_item.items)
        items_bids = list(zip(items, bids))
        return render(request, "silostore_web/auctions_won.html", {
            "items": items,
            "items_bids": items_bids,
            "message": message
        })
